...
const Preferences = () => {
  ...
  const newCompany = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      try {
        ...
        const response = await postMethod('/addCompany', { name: addCompany, csrfToken });
        if (response) {
  ...
  const deleteCompany = async (id: number) => {
    try {
      const response = await postMethod('/deleteCompany', { id, csrfToken });
      if (response) {
        console.log(response);
        getCompanies();
      } else {
  ...
  const newFavoriteHotel = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      try {
        ...
        const response = await postMethod('/addFavoriteHotel', { name: addFavoriteHotel, csrfToken });
        if (response) {
          console.log(response);
          getFavoriteHotels();
          getDislikedHotels();
  ...
  const deleteFavoriteHotel = async (id: number) => {
    try {
      const response = await postMethod('/deleteFavoriteHotel', { id, csrfToken });
      if (response) {
        console.log(response);
        getFavoriteHotels();
      } else {
  ...
  const newDislikedHotel = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      try {
        ...
        const response = await postMethod('/addDislikedHotel', { name: addDislikedHotel, csrfToken });
        if (response) {
          console.log(response);
          getFavoriteHotels();
          getDislikedHotels();
  ...
  const deleteDislikedHotel = async (id: number) => {
    try {
      const response = await postMethod('/deleteDislikedHotel', { id, csrfToken });
      if (response) {
        console.log(response);
        getDislikedHotels();
      } else {
  ...
  const deleteDestination = async (id: number) => {
    try {
      const response = await postMethod('/deleteDestination', { id, csrfToken });
      if (response) {
        console.log(response);
        getDestinations();
      } else {
  ...
  const newLocalAirport = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      try {
        e.preventDefault();
        const response = await postMethod('/addLocalAirport', { airport: localAirport, csrfToken });
        if (response) {
          console.log(response);
          getUserLocalAirport();
        } else {
  ...
  const DeleteLocalAirport = async () => {
    try {
      const response = await deleteMethod('/deleteLocalAirport', { csrfToken });
      if (response) {
        console.log(response);
  ...
  const deleteAirport = async (id: number) => {
    try {
      const response = await deleteMethod('/deleteAirport', { id, csrfToken });
      if (response) {
        console.log(response);
        fetchAirports();
      } else {
  ...
  const handleAddAirport = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      try {
        e.preventDefault();
        const response = await postMethod('/addAirport', { name: newAirport, csrfToken });
        if (response) {
          console.log(response);
          fetchAirports();
        } else {
        ...
        <div className="flex flex-col mt-10 mb-10 gap-10 justify-center items-center">
          <div className="border-2 border-[#562D80] rounded-md w-full max-w-[1200px]">
            <div className="flex flex-col p-10 space-y-3 rounded-md">
              ...
              <p className="text-[#562D80]">
                Trouvez des vols plus facilement en enregistrant votre aéroport local et les autres aéroports où vous vous rendez souvent.
              </p>
              <div>
                ...
                {userLocalAirport !== null ? (
                  <div className="flex flex-row gap-2 bg-[#562D80] text-white rounded-lg p-2 items-center text-sm justify-between my-2 w-full max-w-[400px]">
                    ...
                    <button onClick={DeleteLocalAirport} className="border-2 rounded-full py-1 px-2">
                      X
                    </button>
                  </div>
                ) : (
              ...
              <div className="flex flex-col">
                ...
                <div className="flex flex-row flex-wrap w-full gap-3">
                  {airports?.map((airport) => (
                    <div
                      className="flex flex-row gap-2 bg-[#562D80] text-white rounded-lg p-2 items-center text-xs justify-between my-2"
                      key={airport.id}
                    >
                      <p>{airport.name}</p>
                      <button onClick={() => deleteAirport(airport.id)} className="border-2 rounded-full py-1 px-2">
                        X
                      </button>
                    </div>
                  ))}
          ...
          <div className="border-2 border-[#562D80] rounded-md w-full max-w-[1200px]">
            <div className="flex flex-col p-10 space-y-3 rounded-md">
              ...
              <p className="text-[#562D80]">
                Affinez les résultats de recherche en précisant vos préférences pour les compagnies aériennes.
              </p>
              ...
              <div className="flex flex-row flex-wrap w-full gap-3">
                {companies?.map((company) => (
                  <div
                    className="flex flex-row gap-2 bg-[#562D80] text-white rounded-lg p-2 items-center text-xs justify-between"
                    key={company.id}
                  >
                    <p>{company.name}</p>
                    <button onClick={() => deleteCompany(company.id)} className="border-2 rounded-full py-1 px-2">
                      X
                    </button>
                  </div>
                ))}
          ...
          <div className="border-2 border-[#562D80] rounded-md w-full max-w-[1200px]">
            <div className="flex flex-col p-10 space-y-3 rounded-md">
              ...
              <p className="text-[#562D80]">
                Affinez les résultats de recherche en précisant vos préférences pour les enseignes hôtelières.
              </p>
              ...
              <div className="flex flex-row flex-wrap w-full gap-3">
                {favoriteHotels?.map((favoriteHotel) => (
                  ...
                  >
                    ...
                    <button onClick={() => deleteFavoriteHotel(favoriteHotel.id)} className="border-2 rounded-full py-1 px-2">
                      X
                    </button>
                  </div>
                ))}
              ...
              <p className="text-[#562D80]">Ces options apparaîtront en bas des résultats.</p>
              <div className="flex flex-row flex-wrap w-full gap-3">
                {dislikedHotels?.map((dislikedHotel) => (
                  <div
                    className="flex flex-row gap-2 bg-[#562D80] text-white rounded-lg p-2 items-center text-xs justify-between"
                    key={dislikedHotel.id}
                  >
                    <p>{dislikedHotel.name}</p>
                    <button onClick={() => deleteDislikedHotel(dislikedHotel.id)} className="border-2 rounded-full py-1 px-2">
                      X
                    </button>
                  </div>
                ))}
          ...
          <div className="border-2 border-[#562D80] rounded-md w-full max-w-[1200px]">
            <div className="flex flex-col p-10 space-y-3 rounded-md">
              ...
              <p className="text-[#562D80]">
                Choisissez l’une de vos destinations enregistrées pour trouver des hôtels et locations à proximité.
              </p>
              {ajoutLieu === false ? (
                <>
                  {destinations?.map((destination) => (
                    <React.Fragment key={destination.id}>
                      <div className="flex flex-row gap-2 bg-white rounded-lg p-2 items-center">
                        ...
                        <div className="w-full max-w-[20%]">
                          <button
                            className="text-blue-800 font-semibold mr-5"
                            onClick={() => {
                          ...
                          <button onClick={() => deleteDestination(destination.id)} className="text-blue-800 font-semibold">
                            Supprimer
                          </button>
                        </div>
                      </div>
                  ...
                  <button className="bg-[#562D80] text-white p-1 rounded-md w-[195px] h-[53px]" onClick={() => setAjoutLieu(true)}>
                    Ajouter un lieu
                  </button>
                </>
              ) : (
                <>
                  {editDestination ? (
                    <form
                      onSubmit={(e) => {
                        e.preventDefault();
                        if (addDestination && addDestinationAdress) {
                          modifyDestination(id);
                        }
                      }}
                    ...
                    >
                      <div className="flex flex-row gap-2 my-5">
                        <input
                          type="text"
                          placeholder="Saisissez un hôtel, une adresse ou une attraction"
                          onChange={(e) => setAddDestinationAdress(e.target.value)}
                          value={addDestinationAdress}
                          className="border-2 border-black p-1 w-full max-w-[457px] h-[53px] text-[15px] rounded-md"
                        />
                        <input
                          type="text"
                      ...
                      <div className="flex flex-row gap-2">
                        <button type="submit" className="bg-[#562D80] text-white p-1 rounded-md w-[152px] h-[53px]">
                          Enregister
                        </button>
                        <button onClick={() => setAjoutLieu(false)} className="border-2 border-black p-1 rounded-md w-[152px] h-[53px]">
                          Annuler
                        </button>
                      </div>
                    </form>
